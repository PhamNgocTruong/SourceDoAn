	...
	<div class="container" >
		...
		<sf:form method="post" action="${base }/cart/finish">
			<c:choose>
            		<c:when test="${isLogined }">
            			<div class="form-group">
							...
							placeholder="${userLogined.username}" readonly="readonly" required="required" value="${userLogined.username}"></input>
						</div>	
						<div class="form-group">
						...
						<div class="form-group">
							...
							<input type="email" class="form-control" name="customerEmail" id="customerEmail"
								placeholder="${userLogined.email}" readonly="readonly" required="required" value="${userLogined.email}"></input>
						</div>
            		</c:when>
            		<c:otherwise>
            			<div class="form-group">
			...
			<div class="row">
				<div class="col-md-2">
					<button type="submit"
					class="form-control bg-success text-white buy" value="" 
					onclick="alert('Chúc mừng bạn đã mua hàng thành công!')">Mua hàng</button>
				</div>
				
				<div class="col-md-2"><a class="btn btn-danger" href="${base }/cart/cancel" role="button" style="width:150px">Hủy</a></div>	
			</div>
